...
if ( ! defined( 'ABSPATH' ) ) {
	exit( __( 'The exchange using direct URL is not supported anymore. Please change your exchange URL to http://example.com/?wc1c=exchange.', 'woocommerce-1c' ) );
}

if ( ! defined( 'WC1C_SUPPRESS_NOTICES' ) ) {
	define( 'WC1C_SUPPRESS_NOTICES', false );
}
if ( ! defined( 'WC1C_FILE_LIMIT' ) ) {
	define( 'WC1C_FILE_LIMIT', null );
}
if ( ! defined( 'WC1C_XML_CHARSET' ) ) {
	define( 'WC1C_XML_CHARSET', 'UTF-8' );
}
if ( ! defined( 'WC1C_DISABLE_VARIATIONS' ) ) {
 ...
 * @param null|bool $only_error_code
 */
...
function wc1c_mode_init( $type ) {
	...
	$is_zip = ( 0 === @$status ) || class_exists( 'ZipArchive' );
	if ( ! $is_zip ) {
		wc1c_error( 'The PHP extension zip is required.' );
	}
	...
	if ( WC1C_FILE_LIMIT ) {
		$file_limits[] = wc1c_filesize_to_bytes( WC1C_FILE_LIMIT );
	}
...
function wc1c_mode_file( $type, $filename ) {
	if ( $filename ) {
		...
		if ( ! is_dir( $path_dir ) ) {
			mkdir( $path_dir, 0777, true ) || wc1c_error( sprintf( 'Failed to create directories for file %s', $filename ) );
		}
		...
		if ( is_file( $path ) ) {
			...
			if ( false !== strpos( $temp_header, '<?xml ' ) ) {
				unlink( $path );
			}
		}
...
function wc1c_unpack_files( $type ) {
	...
	if ( 0 !== @$status ) {
		foreach ( $zip_paths as $zip_path ) {
			...
			if ( true !== $result ) {
				wc1c_error( sprintf( 'Failed open archive %s with error code %d', $zip_path, $result ) );
			}

			$zip->extractTo( $data_dir ) || wc1c_error( sprintf( 'Failed to extract from archive %s', $zip_path ) );
			$zip->close() || wc1c_error( sprintf( 'Failed to close archive %s', $zip_path ) );
		}
	}

	foreach ( $zip_paths as $zip_path ) {
		unlink( $zip_path ) || wc1c_error( sprintf( 'Failed to unlink file %s', $zip_path ) );
	}
...
function wc1c_xml_parse_head( $fp ) {
	...
	while ( false !== $buffer ) {
		...
		$is_full = false !== strpos( $buffer, ' СодержитТолькоИзменения="false"' ) || false !== strpos( $buffer, '<СодержитТолькоИзменения>false<' );
		break;
	}
	...
	rewind( $fp ) || wc1c_error( sprintf( 'Failed to rewind on file %s', $filename ) );
...
function wc1c_mode_import( $type, $filename, $namespace = null ) {
	...
	$fp   = fopen( $path, 'r' );
	if ( ! $fp ) {
		wc1c_error( sprintf( 'Failed to open file %s', $filename ) );
	}
	flock( $fp, LOCK_EX ) || wc1c_error( sprintf( 'Failed to lock file %s', $filename ) );
	...
	if ( ! in_array( $namespace, array( 'import', 'offers', 'orders' ), true ) ) {
		wc1c_error( sprintf( 'Unknown import file type: %s', $namespace ) );
	}
	...
	flock( $fp, LOCK_UN ) || wc1c_error( sprintf( 'Failed to unlock file %s', $filename ) );
	fclose( $fp ) || wc1c_error( sprintf( 'Failed to close file %s', $filename ) );
