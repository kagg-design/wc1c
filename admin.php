...
function wc1c_admin_init() {
	...
	if ( ! isset( $wc_product_attributes ) ) {
		return;
	}
...
function wc1c_woocommerce_attribute_taxonomies( $attribute_taxonomies ) {
	if ( is_admin() && 'product_attributes' === @$_GET['page'] ) {
		...
		foreach ( $attribute_taxonomies as $attribute_taxonomy ) {
			$guid = array_search( $attribute_taxonomy->attribute_id, $guids, true );
			if ( false !== $guid ) {
				$attribute_taxonomy->attribute_label .= sprintf( ' [%s: %s]', __( '1C Id', 'woocommerce-1c' ), $guid );
			}
		}
	}
	...
	foreach ( $attribute_taxonomies as $attribute_taxonomy ) {
		$order = array_search( $attribute_taxonomy->attribute_id, $orders, true );
		if ( false !== $order ) {
			$attribute_taxonomy->wc1c_order = $order;
		}
	}
...
function wc1c_manage_product_posts_custom_column( $column ) {
	...
	if ( 'wc1c_guid' === $column ) {
		...
		echo wp_kses_post( $guid ? "<small>$guid</small>" : '<span class="na">–</span>' );
	}
}
...
function wc1c_woocommerce_attribute_deleted( $attribute_id, $attribute_name, $taxonomy ) {
	$guids = get_option( 'wc1c_guid_attributes', array() );
	$guid  = array_search( $attribute_id, $guids, true );
	if ( false === $guid ) {
		return;
	}
...
function wc1c_plugin_row_meta( $plugin_meta, $plugin_file ) {
	if ( WC1C_PLUGIN_BASENAME !== $plugin_file ) {
