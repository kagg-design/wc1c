...
foreach ( $order_posts as $order_post ) {
	$wc1c_order = wc_get_order( $order_post );
	if ( ! $wc1c_order ) {
	...
	$order_line_items = $wc1c_order->get_items();
	...
	$order_shipping_items = $wc1c_order->get_shipping_methods();
	...
	$contragents     = array();
	foreach ( array( 'billing', 'shipping' ) as $wc1c_type ) {
		$contragent = array();

		$name = array();
		foreach ( array( 'first_name', 'last_name' ) as $name_key ) {
			$meta_key = "_{$wc1c_type}_$name_key";
			if ( empty( $order_meta[ $meta_key ] ) ) {
				continue;
			}
		...
		$wc1c_type_country      = "_{$wc1c_type}_country";
		$wc1c_type_country_name = "_{$wc1c_type}_country_name";
		if ( ! empty( $order_meta[ $wc1c_type_country ] ) ) {
			$country_code                          = $order_meta[ $wc1c_type_country ];
			$order_meta[ $wc1c_type_country_name ] = WC()->countries->countries[ $country_code ];
		}

		$full_address = array();
		foreach ( array( 'postcode', 'country_name', 'state', 'city', 'address_1', 'address_2' ) as $address_key ) {
			$meta_key = "_{$wc1c_type}_$address_key";
			if ( ! empty( $order_meta[ $meta_key ] ) ) {
				$full_address[] = $order_meta[ $meta_key ];
			}
		}
		...
		foreach ( $address_items as $address_key => $address_item_name ) {
			...
			$contragent['address'][ $address_item_name ] = $order_meta[ $meta_key ];
		}

		$contragent['contacts'] = array();
		foreach ( $contact_items as $contact_key => $contact_item_name ) {
			$meta_key = "_{$wc1c_type}_$contact_key";
			if ( empty( $order_meta[ $meta_key ] ) ) {
				continue;
			}

			$contragent['contacts'][ $contact_item_name ] = $order_meta[ $meta_key ];
		}

		$contragents[ $wc1c_type ] = $contragent;
	}
	...
	if ( array_key_exists( $wc1c_order->status, $statuses ) ) {
		$order_status_name = $statuses[ $wc1c_order->status ];
	} else {
		$order_status_name = wc_get_order_status_name( $wc1c_order->status );
	}
    ...
    <КоммерческаяИнформация ВерсияСхемы="2.05" ДатаФормирования="<?php echo date( 'Y-m-dTH:i:s', WC1C_TIMESTAMP ); ?>">
		<?php foreach ( $documents as $document ) : ?>
            <Документ>
                ...
                <Контрагенты>
					<?php foreach ( $document['contragents'] as $wc1c_type => $contragent ) : ?>
                        <Контрагент>
                            <Ид>wc1c#user#<?php echo $contragent['user_id']; ?></Ид>
                            <Роль><?php echo $wc1c_type == 'billing' ? 'Плательщик' : 'Получатель'; ?></Роль>
                ...
                <ЗначенияРеквизитов>
                    ...
                    <ЗначениеРеквизита>
                        ...
                        <Значение><?php echo ! in_array( $document['status'], array(
								'on-hold',
								'pending'
							) ) ? 'true' : 'false'; ?></Значение>
                    </ЗначениеРеквизита>
