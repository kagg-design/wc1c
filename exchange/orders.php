...
function wc1c_orders_start_element_handler( $is_full, $names, $depth, $name, $attrs ) {
	...
	if ( 'КоммерческаяИнформация' === @$names[ $depth - 1 ] && 'Документ' === $name ) {
		$wc1c_document = array();
	} elseif ( 'Документ' === @$names[ $depth - 1 ] && 'Контрагенты' === $name ) {
		$wc1c_document['Контрагенты'] = array();
	} elseif ( 'Контрагенты' === @$names[ $depth - 1 ] && 'Контрагент' === $name ) {
		$wc1c_document['Контрагенты'][] = array();
	} elseif ( 'Документ' === @$names[ $depth - 1 ] && 'Товары' === $name ) {
...
function wc1c_orders_character_data_handler( $is_full, $names, $depth, $name, $data ) {
	...
	} elseif ( 'Товар' === @$names[ $depth - 3 ] && 'ХарактеристикиТовара' === @$names[ $depth - 2 ] && 'ХарактеристикаТовара' === @$names[ $depth - 1 ] ) {
		$i = count( $wc1c_document['Товары'] ) - 1;
		$j = count( $wc1c_document['Товары'][ $i ]['ХарактеристикиТовара'] ) - 1;
		@$wc1c_document['Товары'][ $i ]['ХарактеристикиТовара'][ $j ][ $name ] .= $data;
	} elseif ( 'Документ' === @$names[ $depth - 3 ] && 'ЗначенияРеквизитов' === @$names[ $depth - 2 ] && 'ЗначениеРеквизита' === @$names[ $depth - 1 ] ) {
 ...
 *
 * @param WC_Order $order Order.
 * @param array $document_services Document services.
 *
	...
	foreach ( $document_services as $document_service ) {
		foreach ( $shipping_methods as $shipping_method_id => $shipping_method ) {
			...
			if ( ! $shipping_items ) {
				...
				if ( ! $shipping_item_id ) {
					wc1c_error( 'Failed to add shipping to the order' );
				}
			} else {
	...
	if ( isset( $document['Товары'] ) ) {
		foreach ( $document['Товары'] as $i => $document_product ) {
			foreach ( $document_product['ЗначенияРеквизитов'] as $document_product_requisite ) {
				...
				if ( 'Услуга' === $document_product_requisite['Значение'] ) {
