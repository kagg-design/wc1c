...
function wc1c_offers_start_element_handler( $is_full, $names, $depth, $name, $attrs ) {
	...
	} elseif ( 'ТипыЦен' === @$names[ $depth - 1 ] && 'ТипЦены' === $name ) {
		$wc1c_price_types[] = array();
	} elseif ( 'Предложение' === @$names[ $depth - 1 ] && 'Склад' === $name ) {
	...
	} elseif ( 'Цены' === @$names[ $depth - 1 ] && 'Цена' === $name ) {
		$wc1c_price = array();
	}
}
...
function wc1c_offers_character_data_handler( $is_full, $names, $depth, $name, $data ) {
	...
	} elseif ( 'Цены' === @$names[ $depth - 2 ] && 'Цена' === @$names[ $depth - 1 ] ) {
...
function wc1c_offers_end_element_handler( $is_full, $names, $depth, $name ) {
	...
	if ( 'ПакетПредложений' === @$names[ $depth - 1 ] && 'ТипыЦен' === $name ) {
		if ( ! WC1C_PRICE_TYPE ) {
			...
			foreach ( $wc1c_price_types as $price_type ) {
				if ( WC1C_PRICE_TYPE !== @$price_type['Ид'] && WC1C_PRICE_TYPE !== @$price_type['Наименование'] ) {
					continue;
				}
	...
	} elseif ( 'ПакетПредложений' === @$names[ $depth - 1 ] && 'Предложения' === $name ) {
		if ( $wc1c_suboffers ) {
			wc1c_replace_suboffers( $is_full, $wc1c_suboffers );
		}
