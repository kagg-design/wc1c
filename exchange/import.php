...
function wc1c_import_start_element_handler( $is_full, $names, $depth, $name, $attrs ) {
	...
	if ( ! $depth && 'КоммерческаяИнформация' !== $name ) {
		wc1c_error( 'XML parser misbehavior.' );
	} elseif ( 'Классификатор' === @$names[ $depth - 1 ] && 'Группы' === $name ) {
		$wc1c_groups      = array();
		$wc1c_group_depth = - 1;
		$wc1c_group_order = 1;
	} elseif ( 'Группы' === @$names[ $depth - 1 ] && 'Группа' === $name ) {
		$wc1c_group_depth ++;
		$wc1c_groups[] = array( 'ИдРодителя' => @$wc1c_groups[ $wc1c_group_depth - 1 ]['Ид'] );
	} elseif ( 'Группа' === @$names[ $depth - 1 ] && 'Группы' === $name ) {
	...
	} elseif ( 'Классификатор' === @$names[ $depth - 1 ] && 'Свойства' === $name ) {
		$wc1c_property_order       = 1;
		$wc1c_requisite_properties = array();
	} elseif ( 'Свойства' === @$names[ $depth - 1 ] && 'Свойство' === $name ) {
		$wc1c_property = array();
	} elseif ( 'Свойство' === @$names[ $depth - 1 ] && 'ВариантыЗначений' === $name ) {
		$wc1c_property['ВариантыЗначений'] = array();
	} elseif ( 'ВариантыЗначений' === @$names[ $depth - 1 ] && 'Справочник' === $name ) {
		$wc1c_property['ВариантыЗначений'][] = array();
	...
	} elseif ( 'Товар' === @$names[ $depth - 1 ] && 'Группы' === $name ) {
		$wc1c_product['Группы'] = array();
	} elseif ( 'Группы' === @$names[ $depth - 1 ] && 'Ид' === $name ) {
		$wc1c_product['Группы'][] = '';
	} elseif ( 'Товар' === @$names[ $depth - 1 ] && 'Картинка' === $name ) {
		if ( ! isset( $wc1c_product['Картинка'] ) ) {
			$wc1c_product['Картинка'] = array();
		}
		$wc1c_product['Картинка'][] = '';
	} elseif ( 'Товар' === @$names[ $depth - 1 ] && 'Изготовитель' === $name ) {
		$wc1c_product['Изготовитель'] = array();
	} elseif ( 'ХарактеристикиТовара' === @$names[ $depth - 1 ] && 'ХарактеристикаТовара' === $name ) {
		$wc1c_product['ХарактеристикиТовара'][] = array();
	} elseif ( 'ЗначенияСвойств' === @$names[ $depth - 1 ] && 'ЗначенияСвойства' === $name ) {
		$wc1c_product['ЗначенияСвойств'][] = array();
	} elseif ( 'ЗначенияСвойства' === @$names[ $depth - 1 ] && 'Значение' === $name ) {
		$i = count( $wc1c_product['ЗначенияСвойств'] ) - 1;
		if ( ! isset( $wc1c_product['ЗначенияСвойств'][ $i ]['Значение'] ) ) {
			$wc1c_product['ЗначенияСвойств'][ $i ]['Значение'] = array();
		}
		$wc1c_product['ЗначенияСвойств'][ $i ]['Значение'][] = '';
	} elseif ( 'ЗначенияРеквизитов' === @$names[ $depth - 1 ] && 'ЗначениеРеквизита' === $name ) {
		$wc1c_product['ЗначенияРеквизитов'][] = array();
	} elseif ( 'ЗначениеРеквизита' === @$names[ $depth - 1 ] && 'Значение' === $name ) {
		$i = count( $wc1c_product['ЗначенияРеквизитов'] ) - 1;
		if ( ! isset( $wc1c_product['ЗначенияРеквизитов'][ $i ]['Значение'] ) ) {
			$wc1c_product['ЗначенияРеквизитов'][ $i ]['Значение'] = array();
		}
...
function wc1c_import_character_data_handler( $is_full, $names, $depth, $name, $data ) {
	...
	if ( 'Группы' === @$names[ $depth - 2 ] && 'Группа' === @$names[ $depth - 1 ] && 'Группы' !== $name ) {
		@$wc1c_groups[ $wc1c_group_depth ][ $name ] .= $data;
	} elseif ( 'Свойства' === @$names[ $depth - 2 ] && 'Свойство' === @$names[ $depth - 1 ] && 'ВариантыЗначений' !== $name ) {
		@$wc1c_property[ $name ] .= $data;
	} elseif ( 'ХарактеристикиТовара' === @$names[ $depth - 2 ] && 'ХарактеристикаТовара' === @$names[ $depth - 1 ] ) {
		$i = count( $wc1c_product['ХарактеристикиТовара'] ) - 1;
		@$wc1c_product['ХарактеристикиТовара'][ $i ][ $name ] .= $data;
	} elseif ( 'ВариантыЗначений' === @$names[ $depth - 2 ] && 'Справочник' === @$names[ $depth - 1 ] ) {
	...
	} elseif ( 'БазоваяЕдиница' === @$names[ $depth - 2 ] && 'Пересчет' === @$names[ $depth - 1 ] ) {
	...
	} elseif ( 'ЗначенияСвойств' === @$names[ $depth - 2 ] && 'ЗначенияСвойства' === @$names[ $depth - 1 ] ) {
		$i = count( $wc1c_product['ЗначенияСвойств'] ) - 1;
		if ( 'Значение' !== $name ) {
	...
	} elseif ( 'ЗначенияРеквизитов' === @$names[ $depth - 2 ] && 'ЗначениеРеквизита' === @$names[ $depth - 1 ] ) {
		$i = count( $wc1c_product['ЗначенияРеквизитов'] ) - 1;
		if ( 'Значение' !== $name ) {
...
function wc1c_import_end_element_handler( $is_full, $names, $depth, $name ) {
	...
	if ( 'Группы' === @$names[ $depth - 1 ] && 'Группа' === $name ) {
	...
	if ( 'Классификатор' === @$names[ $depth - 1 ] && 'Группы' === $name ) {
	...
	} elseif ( 'Каталог' === @$names[ $depth - 1 ] && 'Товары' === $name ) {
		if ( $wc1c_subproducts ) {
			wc1c_replace_subproducts( $is_full, $wc1c_subproducts );
		}
	...
	} elseif ( ! $depth && 'КоммерческаяИнформация' === $name ) {
...
function wc1c_update_woocommerce_term_meta( $term_id, $meta_key, $meta_value, $prev_value = '' ) {
...
function wc1c_replace_term( $is_full, $guid, $parent_guid, $name, $taxonomy, $order, $use_guid_as_slug = false ) {
	...
	if ( ! $term_id || ! $term ) {
		...
		wc1c_update_woocommerce_term_meta( $term_id, 'wc1c_guid', "$taxonomy::$guid" );
	...
	wc1c_update_woocommerce_term_meta( $term_id, 'wc1c_timestamp', WC1C_TIMESTAMP );
}
...
function wc1c_unique_woocommerce_attribute_name( $attribute_label ) {
	...
	while ( $attribute_name_len > $max_length ) {
...
function wc1c_replace_woocommerce_attribute( $is_full, $guid, $attribute_label, $attribute_type, $order, $preserve_fields ) {
	...
	if ( empty( $is_added ) ) {
		if ( in_array( 'label', $preserve_fields, true ) ) {
			unset( $data['attribute_label'] );
		}
		if ( in_array( 'type', $preserve_fields, true ) ) {
	...
	if ( $is_full ) {
		...
		$order_index = array_search( $attribute_id, $orders, true ) || 0;
...
function wc1c_replace_property( $is_full, $property, $order ) {
	...
	$attribute_type = ( empty( $property['ТипЗначений'] ) || 'Справочник' === $property['ТипЗначений'] || defined( 'WC1C_MULTIPLE_VALUES_DELIMETER' ) ) ? 'select' : 'text';
	...
	if ( 'select' === $attribute_type && ! empty( $property['ВариантыЗначений'] ) ) {
...
function wc1c_replace_post( $guid, $post_type, $is_deleted, $is_draft, $post_title, $post_name, $post_excerpt, $post_content, $post_meta, $category_taxonomy, $category_guids, $preserve_fields ) {
	...
	if ( ! $is_added ) {
		if ( in_array( 'title', $preserve_fields, true ) ) {
			unset( $args['post_title'] );
		}
		if ( in_array( 'excerpt', $preserve_fields, true ) ) {
			unset( $args['post_excerpt'] );
		}
		if ( in_array( 'body', $preserve_fields, true ) ) {
	...
	if ( $is_deleted && 'trash' !== $post->post_status ) {
		wp_trash_post( $post_id );
	} elseif ( ! $is_deleted && 'trash' === $post->post_status ) {
	...
	if ( ! in_array( 'categories', $preserve_fields, true ) ) {
...
function wc1c_replace_product( $is_full, $guid, $product ) {
	...
	$is_deleted = 'Удален' === @$product['Статус'];
	$is_draft   = 'Черновик' === @$product['Статус'];
	...
	foreach ( $product['ЗначенияРеквизитов'] as $i => $requisite ) {
		if ( 'Полное наименование' === $requisite['Наименование'] && @$requisite['Значение'][0] ) {
		...
		} elseif ( 'ОписаниеВФорматеHTML' === $requisite['Наименование'] && @$requisite['Значение'][0] ) {
		...
		foreach ( $product['ЗначенияСвойств'] as $property ) {
			...
			if ( $property_values ) {
				foreach ( $property_values as $property_value ) {
					...
					if ( 'select' === $attribute['attribute_type'] && preg_match( '/^\\w+-\\w+-\\w+-\\w+-\\w+$/', $property_value ) ) {
	...
	if ( ! in_array( 'attributes', $preserve_fields, true ) ) {
		...
		foreach ( $old_product_attributes as $old_product_attribute ) {
			if ( $old_product_attribute['is_taxonomy'] ) {
				...
				$key = array_search( $old_product_attribute, $product_attributes, true );
				if ( false !== $key ) {
	...
	if ( ! in_array( 'attachments', $preserve_fields, true ) ) {
		$attachments = array();
...
function wc1c_clean_woocommerce_attributes( $is_full ) {
	...
	if ( ! empty( $is_deleted ) ) {
		$orders      = get_option( 'wc1c_order_attributes', array() );
		$order_index = array_search( $attribute_id, $orders, true );
		if ( false !== $order_index ) {
			unset( $orders[ $order_index ] );
			update_option( 'wc1c_order_attributes', $orders );
		}
